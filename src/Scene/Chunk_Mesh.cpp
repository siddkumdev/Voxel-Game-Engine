    ...
    for (int x = 0; x < m_ChunkSize; x++) {
        for (int y = 0; y < m_ChunkSize; y++) {
            for (int z = 0; z < m_ChunkSize; z++) {
                ...
                if (!IsActive(x, y, z + 1)) AddFace(glm::vec3(wx, wy, wz + 1), up, right, fwd);
                if (!IsActive(x, y, z - 1)) AddFace(glm::vec3(wx + 1, wy, wz), up, -right, -fwd);
                if (!IsActive(x + 1, y, z)) AddFace(glm::vec3(wx + 1, wy, wz + 1), up, -fwd, right);
                if (!IsActive(x - 1, y, z)) AddFace(glm::vec3(wx, wy, wz), up, fwd, -right);
                if (!IsActive(x, y + 1, z)) AddFace(glm::vec3(wx, wy + 1, wz + 1), -fwd, right, up);
                if (!IsActive(x, y - 1, z)) AddFace(glm::vec3(wx, wy, wz), fwd, right, -up);
            }
        }
    }
